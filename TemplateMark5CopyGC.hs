...
module TemplateMark5CopyGC where
...
evacuateStack heap toHeap stack = (hp, stack { list = as })
  where
    (hp, as) = mapAccumL (uncurry evacuateFrom) (heap, toHeap) $ list stack

evacuateDump :: TiHeap -> TiHeap -> TiDump -> ((TiHeap, TiHeap), TiDump)
evacuateDump = ((,) .) . (,)

evacuateGlobals :: TiHeap -> TiHeap -> TiGlobals -> ((TiHeap, TiHeap), TiGlobals)
evacuateGlobals heap toHeap globals = (hp', globals')
  where
    (hp', globals') = mapAccumL evacuateFrom' (heap, toHeap) globals
    evacuateFrom' (h, th) (name, a1) = (hp2, (name, a2))
      where (hp2, a2) = evacuateFrom h th a1

evacuateFrom :: TiHeap -> TiHeap -> Addr -> ((TiHeap, TiHeap), Addr)
evacuateFrom heap toHeap addr = case node of
  NAp a1 a2                -> case evacuateFrom h1 th1 a1 of
    ((h2, th2), _)           -> case evacuateFrom h2 th2 a2 of
      ((heap', toHeap'), _)    ->  ((heap', toHeap'), a')
    where ((h1, th1), a') = copied
  ...
  NData _tag as              -> case mapAccumL (uncurry evacuateFrom) (h1, th1) as of
    ((heap', toHeap'), _)      ->  ((heap', toHeap'), a')
    where ((h1, th1), a') = copied
  NForward a'                  ->  ((heap, toHeap), a')
  where
    node = hLookup heap addr
    copied = ((hUpdate heap addr $ NForward a', toHeap'), a')
      where (toHeap', a') = hAlloc toHeap node

scavengeHeap :: TiHeap -> TiHeap -> TiHeap
scavengeHeap heap toHeap = foldl (uncurry . hUpdate) toHeap updates
  where
    updates = [ (a, scavenge node)
              | (a, node) <- allocs toHeap
              ]
    scavenge node = case node of
      NAp a1 a2     ->  NAp (lookupNF a1) (lookupNF a2)
      NData tag as  ->  NData tag $ map lookupNF as
      _             ->  node
...
gc state@(output, stack, dump, heap, globals, stats)
  ...
  | otherwise                    =
      (output, stk, d, uncurry scavengeHeap hp, g, stats2)
  where
    (hp2, stk) = evacuateStack heap hInitial stack
    (hp3, d)   = uncurry evacuateDump hp2 dump
    (hp, g)    = uncurry evacuateGlobals hp3 globals
    heapThreshold :: Int
    heapThreshold = 50
